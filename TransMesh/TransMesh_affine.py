...
from MeshPatchEmbed import MeshPatchEmbed
...
'''
def get_3DTransMeshAffine_config():
    '''
    ...
    config.step = 1/12
    config.max_dim = 128
    config.in_chans = config.max_dim*9
...
class MeshSwinTransformer(nn.Module):
    r""" Swin Transformer
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
        Modified by Sam Ehrenstein, MIDAG, UNC Chapel Hill
        This version takes a tokenized mesh.
    Args:
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        self.patch_embed = MeshPatchEmbed(step, max_dim, in_chans, embed_dim)
        ...
        if self.ape:
            # Ignoring this for now, since the tokens have explicit vertex positions already
    ...
    def train(self, mode=True):
        ...
        super(MeshSwinTransformer, self).train(mode)
        self._freeze_stages()
...
# TODO instead of using the built-in affine transform for images, 
# set this up to solve for the 12 parameters of an affine transformation
# matrix that can then be applied to the triangle vertices directly

class TransMeshAffine(nn.Module):
    def __init__(self, config):
        super(TransMeshAffine, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
        embed_dim = config.embed_dim
        self.transformer = MeshSwinTransformer(step=config.step,
        ...
        self.aff_head = nn.Linear(2**13 * 3**3, 100)
        ...
        self.scl_head = nn.Linear(2**13 * 3**3, 100)
        ...
        self.trans_head = nn.Linear(2**13 * 3**3, 100)
        ...
        self.shear_head = nn.Linear(2**13 * 3**3, 100)
    ...
    def forward(self, x):
        ...
        mat, inv_mat, trans = self.affine_trans(source, aff, scl, trans, shr)
        #out, mat, inv_mat = self.affine_trans(source, aff)
        return mat, inv_mat, trans

class MeshAffineTransformer(nn.Module):
    """
    3-D Affine Transformer
    """

    def __init__(self, mode='bilinear', pt_inds=(0,9), face_dims=9):
        super().__init__()
        self.mode = mode
        self.pt_inds = pt_inds
    ...
    def forward(self, src, affine, scale, translate, shear):
        ...
        # pts = torch.t(mat@torch.t(src.verts_packed())+torch.repeat(trans, (1, src.num_pts_per_cloud()[0])))
        # return pytorch3d.structures.Meshes(pts, src.faces_packed()), mat, inv_mat

        '''
        # Affine transformation is y = Ax+b
        # mat is A, trans is b, so we can directly apply them to the
        # mesh vertices
        src_flat = src.reshape(-1, self.face_dims)
        pts_flat = torch.t(src_flat[:,self.pt_inds[0]:self.pt_inds[1]].reshape((-1, 3)))
        pts_trans = torch.t(mat@pts_flat + torch.repeat(trans, (1, pts_flat.shape[1])))
        pts_trans = pts_trans.reshape((-1, 9))
        src_flat[:,self.pt_inds[0]:self.pt_inds[1]] = pts_trans
        # Do other feature transformations here, e.g. cross product
        transformed = src_flat.reshape(src.shape)
        return transformed, mat, inv_mat
        '''
...
CONFIGS = {
    'TransMesh-Affine': get_3DTransMeshAffine_config(),
}

if __name__=="__main__":
    from torchsummary import summary
    model = TransMeshAffine(CONFIGS['TransMesh-Affine'])